...
module.exports = new (class {
  ...
  showDashboard(req, res) {
    ...
    if (req.session.user.role === "admin") {
      ...
      return res.render("admin/dashboard", {
        ...
        msg: "",
        srcImgUser: req.session.user.avatar,
        role: "admin",
      });
    }
    req.params.role = "blogger";
    res.render("blogger/dashboard", {
      user: req.session.user,
      msg: "",
      srcImgUser: req.session.user.avatar,
      role: "blogger",
  ...
  async doUpdateUserInfo(req, res) {
    try {
      ...
      if (req.session.user.role === "admin") {
        ...
        return res.render("admin/dashboard", {
          user: req.session.user,
          msg: "update info doing successfuuly",
          srcImgUser: req.session.user.avatar,
          role: "admin",
        });
      }
  ...
  async doDeleteUser(req, res) {
    try {
      ...
      if (req.session.user.avatar !== "avatarDefault.png") {
        fs.unlinkSync(
          path.join(__dirname, `../public/img/${req.session.user.avatar}`)
        );
      }
  ...
  async doDeleteBlogger(req, res) {
    try {
      ...
      const users = await User.find({ role: { $ne: "admin" } });
      res.render("admin/listOfBloggers", {
  ...
  async creatNewArticle(req, res, next) {
    try {
      ...
      if (req.session.user.role === "admin") {
        ...
        return res.render("admin/dashboard", {
          user: req.session.user,
          msg: "new article created successfully",
          srcImgUser: req.session.user.avatar,
          role: "admin",
        });
      }
  ...
  async showMyArticles(req, res) {
    try {
      ...
      if (req.session.user.role === "admin") {
        req.params.role = "admin";
        return res.render("admin/myArticles", {
          msg: "",
          articles: articles,
  ...
  async showAllArticles(req, res) {
    try {
      ...
      if (req.session.user.role === "admin") {
        return res.render("admin/allArticles", {
          msg: "",
          articles: articles,
          role: "admin",
        });
      }
      res.render("blogger/allArticles", {
        msg: "",
  ...
  async seeMoreArticle(req, res) {
    try {
      ...
      if (req.session.user.role === "admin") {
        return res.render("admin/seeMoreArticle", {
          article,
          msg: null,
          comment,
        });
  ...
  async doChangePassBlogger(req, res) {
    try {
      ...
      const updatePass = await User.findByIdAndUpdate(idUser, {
        password: hashedPass,
      });
      const result = updatePass.save();
      const users = await User.find({ role: { $ne: "admin" } });
      res.render("admin/listOfBloggers", {
        msg: "password of user changed to his phoneNumber successfuly",
        users,
      });
    } catch (err) {
  ...
  async doDeleteArticleOfBloggers(req, res) {
    try {
      ...
      fs.unlinkSync(
        path.join(__dirname, `../public/img/${deleteArticle.image}`)
      );
      const articles = await Article.find({});
      return res.render("wholeArticleForAdmin", {
        msg: "article deleted succussfully",
        articles,
        role: "admin",
      });
    } catch (err) {
